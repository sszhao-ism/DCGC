...
def get_rw_adj(edge_index, norm_dim=1, fill_value=0., num_nodes=None, type='sys'):
    ...
    if type=='sys':   
       ...
       edge_weight = deg_inv_sqrt[indices] * edge_weight * deg_inv_sqrt[indices] #d^{-0.5}AD^{-0.5}
    else: 
       deg_inv_sqrt = deg.pow_(-1)
       edge_weight = deg_inv_sqrt[indices] * edge_weight if norm_dim == 0 else edge_weight * deg_inv_sqrt[indices] #d^{-1}A
...
def adj_normalized(adj, type='sys'):
    ...
    if type=='sys':
        ...
        return d_mat_inv_sqrt.mm(adj).mm(d_mat_inv_sqrt) # (D^{-1/2}AD^{-1/2})
    else: 
        d_inv = torch.pow(row_sum, -1).flatten()
        d_inv[torch.isinf(d_inv)] = 0.
        d_mat_inv = torch.diag(d_inv)
        return d_mat_inv.mm(adj) # (D^{-1}A)
